        ...
        try:
            ...
            for namespace in namespaces:
                for project in self.client.projects.list(search=namespace):
                    ...
                    logging.info(
                        u"auto-create group={} project={}".format(
                            group.name, project.name
                        )
                    )
        except Exception as e:
...
class KubernetesHelper(object):
    ...
    def delete_deprecated_user_role_binding(
        self, user_id, namespace, role_ref
    ):
        try:
            role_bindings = self.client_rbac.list_namespaced_role_binding(
                namespace=namespace,
            ...
            for role_binding in role_bindings.items:
                ...
                logging.info(
                    u"|_ role-binding deprecated name={} namespace={}".format(
                        role_binding.metadata.name, namespace
                    )
                )
        except ApiException as e:
            error = "unable to delete deprecated user role binding :: {}".format(
                eval(e.body)["message"]
            )
            logging.error(error)
        except Exception as e:
            logging.error(
                "unable to delete deprecated user role binding :: {}".format(e)
            )
...
class Gitlab2RBAC(object):
    def __init__(
        self, gitlab, kubernetes, gitlab_auto_create, kubernetes_auto_create
    ):
        self.gitlab = gitlab
...
def main():
    try:
        ...
        if GITLAB_AUTO_CREATE and KUBERNETES_AUTO_CREATE:
            raise Exception(
                "can't have both variables GITLAB_AUTO_CREATE and KUBERNETES_AUTO_CREATE set on True"
            )
